  ...
  item.contacts.forEach(function (contact) {
    ...
    p360(options, function (err, data) {
      if (err) {
        ...
        if (data.GetPrivatePersonsResult.PrivatePersons && data.GetPrivatePersonsResult.PrivatePersons.PrivatePersonBase && data.GetPrivatePersonsResult.PrivatePersons.PrivatePersonBase[0] !== 'undefined') {
